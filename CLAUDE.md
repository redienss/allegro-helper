...
- `.env` (the base directory's, and `~/.allegro-helper/.env`) holds a **real** `OPENAI_API_KEY`. Never commit it; redact it whenever printing either file.
...
**Settings live in `~/.allegro-helper/.env`, not in the base directory** (`Config.globalEnvPath()`, overridable by `ALLEGRO_HELPER_CONFIG_DIR`; the tests redirect it with the `allegrohelper.config.dir` property and must never touch the real one). They belong to the user, not to a folder of photos: they have to survive reinstalling the app or pointing it at a different directory. It also unties a knot: the base directory is itself a setting, and a file found *by way of* the base directory cannot tell you what the base directory is — with the file global, `BASE_DIR` simply lives in it like everything else (`Config.savedBaseDir`, falling back when the saved directory no longer exists). **Launch precedence is argv, then `BASE_DIR`, then the working directory**, and that first step is load-bearing in a way that bit once: `install-desktop-entry.sh` used to write `Exec="$RUN" "$ROOT_DIR"`, so the launcher passed a base directory on every start and the setting saved correctly while appearing to do nothing. The launcher passes no argument now; `run.sh`'s own `cd` keeps the repo as the fallback.

Precedence, lowest to highest: **base-dir `.env` → `~/.allegro-helper/.env` → real environment variable → caller overrides.** The global file outranks a base-dir one because it is what Settings writes — a save a stale project-local file could shadow would look like it had done nothing — while the base-dir file still supplies any key the global one lacks, so an existing checkout keeps working. `Config.migrateLegacyDotenv` seeds the global file from the base directory's on first UI launch (copies, never moves; never overwrites an existing global file), and settings files are chmod 600 because they hold an API key. See `GlobalSettingsTest`.

`Config.forBaseDir(baseDir)` derives every path from the base directory. The UI's own controls win over the files: `currentConfig()` passes the photo directory field as `MTP_GLOB_PATTERN`, the series dropdown as `SERIES_RECOGNITION`, and the two Retouch Preview sliders as `BRIGHTNESS_STRENGTH` / `CONTRAST_STRENGTH`. Keys: `CSV_PATH`, `RAW_PHOTOS_DIR`, `OFFERS_DIR`, `MTP_UID`, `MTP_GLOB_PATTERN`, `SERIES_GAP_THRESHOLD_SECONDS`, `SERIES_RECOGNITION` (`auto` | `single` | `subfolders`), `BRIGHTNESS_STRENGTH`, `CONTRAST_STRENGTH`, `OCR_LANGUAGES`, `OPENAI_API_KEY`, `OPENAI_MODEL`, `OPENAI_BASE_URL`, `OPENAI_SYSTEM_PROMPT`, `OPENAI_USER_PROMPT`, `CHROME_BIN`, `CHROME_PROFILE_DIR`.

File > Settings has four pages. *Photos* sets the **default base directory** (`BASE_DIR`; a path that is not a directory clears the key rather than being saved, since the app would open on it and every derived path would be broken), the **default photo directory** (`MTP_GLOB_PATTERN`) and the **default series recognition mode** (`SERIES_RECOGNITION`) (removed when it is back to `auto`, the built-in default) — the dropdown in the Photos section is then seeded from it at startup, which is the point: the mode a user works in most stops being a per-launch chore. Applying it also points the live dropdown at the new mode, via a callback fired **only when that default actually changed** — firing on every apply would reset a mode picked for the session by someone who came in to change the theme.

The OpenAI keys are editable in File > Settings > OpenAI API, which writes them to the global settings file via `Config.updateDotenv` (only values differing from the built-in defaults; multi-line prompts are stored double-quoted with `\n` escapes). The prompt defaults are `GenerateDescription.SYSTEM_PROMPT` / `USER_PROMPT`; the user prompt is a template whose `{{OFFER_JSON}}` placeholder is replaced with the offer JSON.
