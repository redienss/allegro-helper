...
**Settings live in `~/.allegro-helper/.env`, not in the base directory** (`Config.globalEnvPath()`, overridable by `ALLEGRO_HELPER_CONFIG_DIR`; the tests redirect it with the `allegrohelper.config.dir` property and must never touch the real one). They belong to the user, not to a folder of photos: they have to survive reinstalling the app or pointing it at a different directory. It also breaks a circularity — the base directory is itself a setting, and a file found *by way of* the base directory cannot tell you what the base directory is. That circularity is why the base directory alone lives in `Preferences` (`ui/BaseDir`) rather than in any `.env`; launch precedence is argv, then that saved default, then the working directory.
